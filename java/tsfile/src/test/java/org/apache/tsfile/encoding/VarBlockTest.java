...
//            }
    ...
    public static int Encoder(int[] data, byte[] encoded_result) {
...
//        int num_blocks = data_length / block_size;
        ...
        int previous_value = data[0] - value_delta_min;
        int cur_count = 1;
        int j = 1 ;
        int[] bit_width_list = new int[data_length];
        int[] block_size_list = new int[data_length];

        for (; j < data_length; j++) {
            data[j] -= value_delta_min;
            int current_value = data[j];
...
//        }
    ...
    public void testBP() throws IOException {
        ...
        String outputPath = output_parent_dir + "test_vb.csv";
        ...
        for (File file : csvFiles) {
            ...
            for (int repeat = 0; repeat < repeatTime; repeat++) {
                length = Encoder(data2_arr, encoded_result);
            }
