...
public class IntVlbpEncoder extends Encoder {
  ...
  public IntVlbpEncoder(TSEncoding encoding) {
    super(encoding);
  ...
  public void flush(ByteArrayOutputStream out) throws IOException {
    ...
    long[] longs = new long[valueCount];
    for (int i = 0; i < valueCount; i++) {
      longs[i] = values[i];
    }
    byte[] encoded = VlbpCodec.encode(longs, strategy, transform);
